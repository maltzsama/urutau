...
// RV-01 (CR-069): the columnar enrich seam keeps deletes and backfills
// their key from the wire record's key columns — no PK argument, no bridge.
...
func TestEnrichBatchKeepsDeletesColumnar(t *testing.T) {
	...
	// The key-only delete carries its PK value — RecordFromChanges backfills
	// the PK column from the key tuple on encode.
...
func TestEnrichBatchSameSchemaAcrossEmptyAndHotReference(t *testing.T) {
	...
	mkBatch := func(t *testing.T, id int64) *dpint.Batch {
		...
		rec, err := transport.RecordFromChanges(changes, wire, nil)
		if err != nil {
